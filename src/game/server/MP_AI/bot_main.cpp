...
CON_COMMAND_F( gabeplus_bot, "Add a bot.", 0 /*FCVAR_CHEAT*/ )
{
...
CBasePlayer *BotPutInServer( bool  bFrozen )
{
	...
	if (!pPlayer)
	{
		Warning("Bot entity creation failed!\n");
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove( CBasePlayer *fakeclient, CUserCmd &cmd, float frametime )
{
	if ( !fakeclient )
		return;
