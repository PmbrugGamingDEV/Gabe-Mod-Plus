...
class C_JBBeamQuadratic : public CDefaultClientRenderable
{
public:
	C_JBBeamQuadratic();
	...
	{
		AngleMatrix(vec3_angle, m_worldPosition, z);
		return z;
	}
...
{
	DECLARE_CLASS(C_JBWeaponGravityGun, C_BaseHLCombatWeapon);
public:
...
private:
	C_JBWeaponGravityGun(const C_JBWeaponGravityGun&);

	C_JBBeamQuadratic	m_beam;
};

STUB_WEAPON_CLASS_IMPLEMENT(weapon_jbphysgun, C_JBWeaponGravityGun);

IMPLEMENT_CLIENTCLASS_DT(C_JBWeaponGravityGun, DT_JBWeaponGravityGun, CJBWeaponGravityGun)
...
C_JBBeamQuadratic::C_JBBeamQuadratic()
{
	m_pOwner = NULL;
}

void C_JBBeamQuadratic::Update(C_BaseEntity* pOwner)
{
	m_pOwner = pOwner;
	if (m_active)
	{
...
int	C_JBBeamQuadratic::DrawModel(int)
{
	Vector points[3];
	QAngle tmpAngle;
